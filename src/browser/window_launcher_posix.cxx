...
bool FindInPath(const char *filename, std::string &out) {
  ...
  while (true) {
    ...
    std::string_view path_item =
        is_last ? std::string_view(path)
                : std::string_view(path, (size_t)(next_colon - path));
...
// see #34 for why this function exists and why it can't be run between
// fork-exec or just run `env`. true on success, false on failure, out is
// undefined on failure, you know the drill. java_home should be the result of
// getenv("JAVA_HOME") and therefore is allowed to be null
...
bool FindWine(std::string &out) {
  if (FindInPath("umu-run", out))
    return true;
  return FindInPath("wine", out);
}

// takes a custom launch command string, splits it into individual args,
// replaces "%command%" with the args in original_command, and writes the
// resulting argument list to buf. returns true on success, false on error.
// original_command must be a null-terminated list, and output will be
// null-terminated (as long as the function returns true).
bool ResolveLaunchCommand(char *launch_command, char **original_command,
                          char **buf) {
  size_t out_index = 0;
...
Browser::Launcher::LaunchRs3Deb(CefRefPtr<CefRequest> request,
                                std::string_view query) {
  /* strings that I don't want to be searchable, which also need to be mutable
   * for passing to env functions */
  // PULSE_PROP_OVERRIDE=
  char env_pulse_prop_override[] = {
      97,  112, 112, 108, 105, 99,  97,  116, 105, 111, 110, 46,  110, 97,
      109, 101, 61,  39,  82,  117, 110, 101, 83,  99,  97,  112, 101, 39,
      32,  97,  112, 112, 108, 105, 99,  97,  116, 105, 111, 110, 46,  105,
      99,  111, 110, 95,  110, 97,  109, 101, 61,  39,  114, 117, 110, 101,
      115, 99,  97,  112, 101, 39,  32,  109, 101, 100, 105, 97,  46,  114,
      111, 108, 101, 61,  39,  103, 97,  109, 101, 39,  0};
  // SDL_VIDEO_X11_WMCLASS=
  char env_wmclass[] = {82, 117, 110, 101, 83, 99, 97, 112, 101, 0};
  constexpr char tar_xz_inner_path[] = {
      46,  47,  117, 115, 114, 47,  115, 104, 97,  114, 101, 47,
      103, 97,  109, 101, 115, 47,  114, 117, 110, 101, 115, 99,
      97,  112, 101, 45,  108, 97,  117, 110, 99,  104, 101, 114,
      47,  114, 117, 110, 101, 115, 99,  97,  112, 101, 0};
  constexpr char tar_xz_icons_path[] = "./usr/share/icons/";

  const CefRefPtr<CefPostData> post_data = request->GetPostData();

  // get local mutable copy of HOME env string - we redirect the game's HOME
  // into our data dir
  ...
  // if there was a "hash" in the query string, we need to save the new game exe
  // and the new hash
  if (has_hash) {
    ...
    // use libarchive to extract data.tar.xz into memory from the supplied .deb
    // (ar compression format)
    ...
    if (!entry_found) {
      // The .deb file is valid but does not contain "data.tar.xz" according to
      // libarchive
      archive_read_close(ar);
      archive_read_free(ar);
      delete[] deb;
    ...
    // open data.tar.xz and extract any files into memory that we're interested
    // in
    ...
    while (true) {
      ...
      if (strcmp(entry_pathname, tar_xz_inner_path) == 0) {
        ...
        written = 0;
        int file = open(this->rs3_elf_path.c_str(),
                        O_WRONLY | O_CREAT | O_TRUNC, 0755);
        if (file == -1) {
          // failed to open game binary file on disk - probably in use or a
          // permissions issue
          delete[] game;
          QSENDSTR("Failed to save executable; if the game is already running, "
        ...
        close(file);
        delete[] game;
      } else if (strncmp(entry_pathname, tar_xz_icons_path,
                         strlen(tar_xz_icons_path)) == 0) {
        // found an icon - save this to the icons directory, maintaining its
        // relative path
        ...
        if (entry_pathname[entry_pathname_len - 1] == '/') {
          ...
          while (written < icon_size) {
            written +=
                archive_read_data(ar, icon + written, icon_size - written);
          }
          written = 0;
          int file =
              open(icon_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0755);
          if (file == -1) {
            // failing to save an icon is not a fatal error, but probably
    ...
    if (!entry_found) {
      // data.tar.xz was valid but did not contain a game binary according to
      // libarchive
  ...
  if (has_launch_command) {
    QSENDSYSTEMERRORIF(
        !ResolveLaunchCommand(launch_command.data(), args, arg_buffer));
    argv = arg_buffer;
  }
  ...
  if (pid == 0) {
    ...
    if (has_jx_session_id)
      setenv("JX_SESSION_ID", jx_session_id.data(), true);
    if (has_jx_character_id)
      setenv("JX_CHARACTER_ID", jx_character_id.data(), true);
    if (has_jx_display_name)
      setenv("JX_DISPLAY_NAME", jx_display_name.data(), true);
#if defined(BOLT_PLUGINS)
    if (plugin_loader) {
  ...
  if (has_hash) {
    size_t written = 0;
    int file = open(this->rs3_elf_hash_path.c_str(),
...
CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchRs3Exe(CefRefPtr<CefRequest> request,
                                std::string_view query) {
  QSENDSTR(".exe is not supported on this platform", 400);
}

CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchRs3App(CefRefPtr<CefRequest> request,
                                std::string_view query) {
  QSENDSTR("Mac binaries are not supported on this platform", 400);
}

CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchOsrsExe(CefRefPtr<CefRequest> request,
                                 std::string_view query) {
  const CefRefPtr<CefPostData> post_data = request->GetPostData();
  ...
  // and the new hash
  if (has_hash) {
    QSENDBADREQUESTIF(post_data == nullptr ||
                      post_data->GetElementCount() != 1);
    CefPostData::ElementVector vec;
    post_data->GetElements(vec);
    ...
    size_t written = 0;
    int file =
        open(this->osrs_exe_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0755);
    if (file == -1) {
      // failed to open game binary file on disk - probably in use or a
      // permissions issue
      delete[] exe;
      QSENDSTR("Failed to save exe; if the game is already running, close it "
               "and try again",
               500);
    }
  ...
  if (pid == 0) {
    ...
    if (has_jx_display_name)
      setenv("JX_DISPLAY_NAME", jx_display_name.data(), true);
    // game id from steam for OSRS. This allows umu to apply any necessary
    // protonfixes
    setenv("GAMEID", "1343370", true);
    // tell umu to use the latest GE Proton it can find, which will perform
    // better in most cases
    setenv("PROTONPATH", "GE-Proton", true);
    // tell umu and wine to store game data in data_dir/pfx
    std::string wineprefix = this->data_dir.string() + "/pfx";
    setenv("WINEPREFIX", wineprefix.c_str(), true);

    // This is needed due to proton's pressure vessel changing the mount point
    // of /app to /run/parent/app inside of pressure vessel. It tries to chdir
    // back to /app/opt/bolt-launcher which will no longer exist. Chdir to
    // data_dir works to work around this.
    if (chdir(this->data_dir.c_str())) {
      fmt::print("[B] new process was unable to chdir: {}\n", errno);
      exit(1);
    }
  ...
  if (has_hash) {
    size_t written = 0;
    int file = open(this->osrs_exe_hash_path.c_str(),
                    O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (file == -1) {
...
CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchOsrsApp(CefRefPtr<CefRequest> request,
                                 std::string_view query) {
  QSENDSTR("Mac binaries are not supported on this platform", 400);
}

CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchRuneliteJar(CefRefPtr<CefRequest> request,
                                     std::string_view query, bool configure) {
  const CefRefPtr<CefPostData> post_data = request->GetPostData();
  ...
  // path to runelite.jar will either be a user-provided one or one in our data
  // folder, which we may need to overwrite with a new user-provided file
  ...
  if (has_jar_path) {
    ...
    // if there was an "id" in the query string, we need to save the new jar and
    // hash
    if (has_id) {
      QSENDBADREQUESTIF(post_data == nullptr ||
                        post_data->GetElementCount() != 1);
      ...
      if (file == -1) {
        ...
        // permissions issue
        delete[] jar;
        QSENDSTR("Failed to save JAR; if the game is already running, close it "
                 "and try again",
                 500);
      }
  ...
  if (!FindJava(getenv("JAVA_HOME"), java)) {
    QSENDSTR("Couldn't find Java: JAVA_HOME is either unset or does not point "
             "to a Java binary, and no binary named \"java\" exists in PATH.",
             500);
  }
  ...
  if (has_id) {
    size_t written = 0;
    int file = open(this->runelite_id_path.c_str(),
                    O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (file == -1) {
      QSENDSTR("OK, but unable to save id file", 200);
      const char *data = "OK, but unable to save id file\n";
...
CefRefPtr<CefResourceRequestHandler>
Browser::Launcher::LaunchHdosJar(CefRefPtr<CefRequest> request,
                                 std::string_view query) {
  const CefRefPtr<CefPostData> post_data = request->GetPostData();
  ...
  std::string java;
  if (!FindJava(getenv("JAVA_HOME"), java)) {
    QSENDSTR("Couldn't find Java: JAVA_HOME does not point to a Java binary",
             400);
  }
  ...
  // if there was a "version" in the query string, we need to save the new jar
  // and hash
  if (has_version) {
    QSENDBADREQUESTIF(post_data == nullptr ||
                      post_data->GetElementCount() != 1);
    ...
    size_t written = 0;
    int file =
        open(this->hdos_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0755);
    if (file == -1) {
      // failed to open game binary file on disk - probably in use or a
      // permissions issue
      delete[] jar;
  ...
  char arg_jar[] = "-jar";
  char *args[] = {
      java.data(), arg_user_home.data(), arg_app_user_home.data(),
      arg_jar,     path_str.data(),      nullptr,
  };
  char **argv = args;
  char *arg_buffer[MAXARGCOUNT];
  if (has_launch_command) {
  ...
  if (has_version) {
    size_t written = 0;
    int file = open(this->hdos_version_path.c_str(),
                    O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (file == -1) {
    ...
    while (written < version.size()) {
      written +=
          write(file, version.c_str() + written, version.size() - written);
    }
    close(file);
  }
  QSENDOK();
